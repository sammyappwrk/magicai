...
class AbsenseController extends Controller
{
    ...
    public function absenseList(){
        $absense_arr = Absense::all();
        return view('absenses.index',compact('absense_arr'));
    }
    public function absenseFrontEndList($parameter){
        return $absense_arr = Absense::where('section_name', $parameter)->where('status', '1')->get();
    }
    public function addAbsense(Request $req) {
        $inputs = request()->all();
         $message = $inputs['message'];
         $prev_id = $inputs['prev_id'];
        $selected_value = json_decode($inputs['selected_value'],true);
        if($prev_id > 0) {
            Absense::where('id', $prev_id)
          ->update(['content' => $message]);
        } else {
          $validateUpdate =  Absense::whereIn('section_name', $selected_value)
            ->update(['content' => $message]);
        }
       
        return 'done';
    }
    public function getSpecificAbsense(Request $req) {
        $inputs = request()->all();
         $id = $inputs['id'];
        $absense_arr = Absense::where('id', $id)->get();
        $escapedScriptTag = $absense_arr[0]['content'];
        $response = [
            'content' => $escapedScriptTag,
        ];
